...
if __name__=='__main__':
    ...
    try:
        while True:
            ...
            K = color_frame.profile.as_video_stream_profile().get_intrinsics()
            with open('demo_data/' + args.dataset_name + "/cam_K.txt", "w") as f:
                f.write(f'{K.fx} 0 {K.ppx}\n')
                f.write(f'0 {K.fy} {K.ppy}\n')
                f.write(f'0 0 1\n')
            ...
            if key & 0xFF == ord('s'):
                cv2.imwrite(os.path.join(rgb_dir, "%04i.png" % count), color_image)
                cv2.imwrite(os.path.join(depth_dir, "%04i.png" % count), depth_image)
                count += 1
